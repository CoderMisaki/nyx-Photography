...
const createClothMaterial = () => {
    ...
    `,
      ...
      void main() {
        ...
        // Blur ringan - 3x3 = 9 taps (sebelumnya 5x5=25) biar tidak berat di HP
        if (blurAmount > 0.15) {
          ...
          color = mix(color, blurred / total, clamp(blurAmount/3.0, 0.0, 1.0));
        }
...
function ImagePlane({
	...
	return (
		...
		>
			<planeGeometry args={[1, 1, isMobile ? 12 : 20, isMobile ? 12 : 20]} />
		</mesh>
	);
}
...
function GalleryScene({
	...
	const handleTouchMove = useCallback(
		(event: TouchEvent) => {
			if (touchStartY.current !== null && event.touches.length > 0) {
				const deltaY = touchStartY.current - event.touches[0].clientY;
				const deltaX = (touchStartX.current ?? 0) - event.touches[0].clientX;
				const movement = Math.abs(deltaY) > Math.abs(deltaX) ? deltaY : deltaX;
				setScrollVelocity((prev) => prev + movement * 0.03 * speed);
				touchStartY.current = event.touches[0].clientY;
				touchStartX.current = event.touches[0].clientX;
				setAutoPlay(false);
				lastInteraction.current = Date.now();
	...
	}, [handleWheel, handleKeyDown, handleTouchStart, handleTouchMove, handleTouchEnd, handleCanvasClick]);
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-surface-soft p-6">
			<p className="text-secondary mb-4 text-xs uppercase tracking-widest font-mono">
...
export default function InfiniteGallery({
	...
	const isMobileCanvas = typeof window !== 'undefined' ? window.matchMedia('(max-width: 768px)').matches : false;
	return (
		<div className={className} style={style}>
			<Canvas
				camera={{ position: [0, 0, 0], fov: 55 }}
				dpr={[1, isMobileCanvas ? 1.2 : 1.8]}
				gl={{ antialias: !isMobileCanvas, alpha: true, powerPreference: 'low-power' }}
				performance={{ min: 0.5 }}
			>
				<GalleryScene
					images={images}
