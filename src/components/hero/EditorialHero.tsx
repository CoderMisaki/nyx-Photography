...
import { CategoryId, CATEGORIES, ALL_PHOTOGRAPHS } from '@/data/photography';
...
export const EditorialHero: React.FC<EditorialHeroProps> = ({
  ...
  const pickRandom = (exclude: number) => {
    const total = ALL_PHOTOGRAPHS.length;
    if (total <= 1) return 0;
    let r = Math.floor(Math.random() * total);
    let guard = 0;
    while (r === exclude && guard < 10) {
  ...
  useEffect(() => {
    const delays = [100, 200, 300, 400, 500]; // 0.1s → 0.2s → 0.3s → 0.4s → 0.5s → loop 0.1

    const schedule = () => {
      const d = delays[delayPosRef.current % delays.length];
      timeoutRef.current = window.setTimeout(() => {
        if (document.hidden || isFadingRef.current) {
          delayPosRef.current = (delayPosRef.current + 1) % delays.length;
          schedule();
          return;
        }
        const rnd = pickRandom(currentIdxRef.current);
        const img = new Image();
        img.src = ALL_PHOTOGRAPHS[rnd].src;
        const startFade = () => {
          setIncomingIdx(rnd);
        ...
        if ('decode' in img) {
          ...
          (img as HTMLImageElement).onload = (img as HTMLImageElement).onerror = startFade as any;
        }
  ...
  const current = ALL_PHOTOGRAPHS[currentIdx];
  const incoming = incomingIdx !== null ? ALL_PHOTOGRAPHS[incomingIdx] : null;

  const captionLocation = current.location.toUpperCase();
  const captionCamera = `${current.camera} • ${current.shutter}`;
  ...
  return (
    <section className="relative min-h-[92vh] flex flex-col justify-between pt-24 md:pt-32 pb-12 px-6 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 my-auto py-8 items-center">
        ...
        {/* Right Column: Hero Cover Photo Display - siklus 0.1→0.5s, loader sinkron tema */}
        <div className="lg:col-span-5 relative">
          <div className="relative aspect-[4/5] sm:aspect-[3/4] lg:aspect-[4/5] bg-surface-soft overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-border isolate">
            {/* Base layer - selalu ada, tidak ikut transition layout */}
            <img
              src={current.src}
              alt={current.title}
              decoding="async"
              fetchPriority="high"
              onLoad={() => markImageCached(current.src)}
              className="absolute inset-0 w-full h-full object-cover"
              style={{ transform: 'translateZ(0)', backfaceVisibility: 'hidden' as const }}
            />
            {/* Incoming layer - crossfade opacity only, GPU accelerated */}
            {incoming && (
              <img
                src={incoming.src}
                alt=""
                aria-hidden
                decoding="async"
                onLoad={() => markImageCached(incoming.src)}
            ...
            {/* Subtle Gradient - tetap di atas, FEATURED dihapus sesuai request */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6 text-white pointer-events-none">
              <span className="font-mono text-[10px] uppercase tracking-widest text-white/80">
                {current.category.toUpperCase()} SERIES
              </span>
              <p className="font-serif text-lg italic">{current.title}</p>
            </div>
          </div>

          {/* Museum Caption Underneath - fixed height biar tidak layout shift saat ganti */}
          <div className="mt-3 flex items-center justify-between text-[10px] font-mono text-muted uppercase min-h-[14px]">
            <span className="truncate pr-2">{captionLocation}</span>
            <span className="shrink-0">{captionCamera}</span>
          </div>
        </div>
      </div>
